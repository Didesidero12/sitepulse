...
export default function ClaimTicket() {
  ...
  if (loading) return <p className="text-6xl text-center mt-40 text-white">Loading...</p>;
      ...
      {!claimed ? (
        ...
        <>
          ...
          <Link href={`/driver?ticketId=${ticket.firestoreId}`}>
            <button className="bg-cyan-600 hover:bg-cyan-700 text-white text-6xl font-bold py-20 px-48 rounded-3xl shadow-2xl transition-all hover:scale-110">
              PROCEED TO TRACKING
            </button>
          </Link>
        </>
