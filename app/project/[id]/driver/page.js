...
export default function DriverView() {
  ...
  const siteLocation = { lat: 45.5231, lng: -122.6765 }; // Replace with real site GPS from project
  ...
  const getDistance = (loc1, loc2) => {
    ...
    const a = Math.sin(dLat / 2) * Math.sin(dLat / 2) + Math.sin(dLon / 2) * Math.sin(dLon / 2) * Math.cos(toRad(loc1.lat)) * Math.cos(toRad(loc2.lat));
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white flex flex-col">
      <div className="bg-green-600 p-8 text-center">
        <h1 className="text-5xl font-bold">DRIVER MODE</h1>
        <p className="text-3xl">Project {id}</p>
      </div>

      <div className="p-8 space-y-8">
        <div className="bg-gray-800 p-8 rounded-2xl text-center">
          <h2 className="text-4xl font-bold text-green-400 mb-4">DELIVERY</h2>
          <p className="text-5xl mb-2">{delivery?.material}</p>
          <p className="text-3xl">{delivery?.qty}</p>
          {delivery?.needsForklift && <p className="text-red-400 text-3xl mt-4">FORKLIFT NEEDED</p>}
        </div>

        <div className="bg-gray-800 p-8 rounded-2xl text-center">
          <h2 className="text-4xl font-bold text-orange-400 mb-4">ETA</h2>
          <p className="text-9xl font-black">{delivery?.eta}</p>
        </div>

        <div id="map" className="h-96 rounded-2xl bg-gray-800"></div>

        <button onClick={() => setTracking(!tracking)} className={`${tracking ? 'bg-red-600' : 'bg-green-600'} hover:opacity-90 p-8 text-5xl font-bold rounded-2xl w-full`}>
          {tracking ? "STOP TRACKING" : "START TRACKING →"}
        </button>

        <button className="bg-blue-600 hover:bg-blue-500 p-8 text-5xl font-bold rounded-2xl w-full">
          I’VE ARRIVED
        </button>
      </div>
    </div>
